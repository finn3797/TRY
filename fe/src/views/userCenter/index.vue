				...
				<el-card shadow="never">
					<div class="user-info">
						<div class="user-info-top">
							...
							<h2>{{ form.name || '-' }}</h2>
							<p>{{ form.about || '无签名' }}</p>
							<el-button type="primary" round icon="el-icon-collection-tag" size="large">{{ roleString }}</el-button>
						</div>
							...
							<el-timeline style="margin-top:20px;padding-left:10px;">
								...
								>
									<div class="activity-item">
										...
										<el-tag v-if="activity.mod">{{ activity.mod }}</el-tag>
										{{ activity.describe }}
									</div>
								</el-timeline-item>
							</el-timeline>
...
export default {
	...
	methods: {
		loadUser() {
			const u = this.$TOOL.data.get("USER_INFO")
			// 避免双向影响
			// this.imageUrl = u.avatar
			this.form = {
				...u
			}
			this.roleString = 'administrator'
		},
		doUpdate() {
			console.log(this.form, 'doUpdate')
		},
		handleAvatarSuccess(response, uploadFile) {
			console.log(response, 'vvvv', uploadFile)
